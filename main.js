...
propertyFiltro1 = propertyFiltro1.filter(function(exc) { // filtramos para quitar propiedades que no van en filtro
  if(exc != 'size' && exc != 'img'  && exc != 'about'&& exc != 'generation' && exc != 'pokemon-rarity' && exc != 'encounter' && exc != 'spawn-chance' && exc  != 'buddy-distance-km'){
    return true
     ...
     filtrar(); //Muestra resultado para primer pokemon



function addOptions(prop) { //Funcion Llena Menu 1
...
function addPropFiltro2() { //Llena Menu 2
  ...
  if (optionFiltro1==="evolution"){ // Si opcion filtrados 1 es Evolucion Muestar Menu 3 con nombres de pokemon
 ...
 fnBoxPuntBase(optionFiltro1); //Funcion Oculta/Muestra Boton y textbox de puntos base 
  
  
  for (let i=0; i<=data.pokemon.length-1; i++){
    if(typeof data.pokemon[i][optionFiltro1] == "string"){ //Llena el menu 2 con las opciones de nombre, numero y eggs
      console.log("nombre");
      ...
      if(typeof data.pokemon[i][optionFiltro1].length == "undefined"){ //Llena el menu 2 con Estadisticas y Evolucion
        console.log("Estadi/Evolu");
      ...
      for(let j=0; j<=data.pokemon[i][optionFiltro1].length-1;j++){
        ...
        if(typeof data.pokemon[i][optionFiltro1][j] == "object"){//Llena menu 2 con Movimiento Rapido, Ataque especial
        ...
        else{//Llena Menu 2 con Tipo, Resistencia, Debilidad
          tipo = Object.values(data.pokemon[i][optionFiltro1][j]);
  ...
  for (let value in subcategoria) {
    ...
    if ((typeof data.pokemon[value][optionFiltro1]) === "string"){//filtrar traductor num
...
function filtrar() {
  ...
  var listPok = objInfFilOrg.map(function(pok){
    ...
    return '<li> <div class="boxTarjeta"><div class="boxImg"><figure><a href=pokemon.html><img src='+pok.img+'></a></figure></div><div class="boxInf"><p class="namePok">'+"N"+Number(pok.num)+"&nbsp;"+pok.name+'</p><p><span class="texto">Tipo:</span>'+traductor(pok.type)+' </p><p><span class="texto">Fuerte Contra:</span> '+traductor(pok.resistant)+'</p> <p> <span class="texto">Debil Contra:</span> '+traductor(pok.weaknesses)+'</p><p><span class="texto">Ataque Base:</span> '+pok.stats['base-attack']+'</p><p> <span class="texto">Defensa Base:</span> '+pok.stats['base-defense']+'</p></div></div></li>' 
  })
